...
func NewRedisClient(url, port, pass string) RedisClient {
...
func (rd *RedisClient) InitRedisConnection(ctx context.Context) (*redis.Client, error) {

	host := rd.rediscfg.Url + ":" + rd.rediscfg.Port
	conn := redis.NewClient(&redis.Options{
		Addr:     host,
		Password: rd.rediscfg.Pass,
	})
...
func (rd *RedisClient) PutCacheTtl(ctx context.Context, conn *redis.Client, cacheName string, value Cache, ttl time.Duration) error {

	err := conn.Set(ctx, cacheName, value, ttl).Err()

	if err != nil {
		return errorRedis(conn.Options().Addr, "PUT", cacheName, err)
	}
...
// put complex cache (ex: for suggestion)
// param ctx, redisclient, cachename string, value any, ttl time.Duration
// e.g:
// store cache for 30 sec, 30*time.Second
// store cache for 10 minute, 10*time.Minute
// store cache no expiration time, use redis.KeepTTL
func (rd *RedisClient) StoreComplexCache(ctx context.Context, conn *redis.Client, cacheName string, value Cache, ttl time.Duration) error {
	err := conn.HSet(ctx, cacheName, value, ttl).Err()
	if err != nil {
		return errorRedis(conn.Options().Addr, "PUT", cacheName, err)
	}
...
// return any, error
func (rd *RedisClient) GetCache(ctx context.Context, conn *redis.Client, cacheName string) (Cache, error) {

	res, err := conn.Get(ctx, cacheName).Result()
	if err != nil {
		return nil, errorRedis(conn.Options().Addr, "GET", cacheName, err)
	}
	return res, nil
}
...
func (rd *RedisClient) GetHashCache(ctx context.Context, conn *redis.Client, cacheName, field string) (Cache, error) {
	var res Cache
	if field == "" {
		data, err := conn.HGetAll(ctx, cacheName).Result()
		if err != nil {
			return nil, errorRedis(conn.Options().Addr, "GET", cacheName, err)
		}
		res = data
	} else {
		data, err := conn.HGet(ctx, cacheName, field).Result()
		if err != nil {
			return nil, errorRedis(conn.Options().Addr, "GET", cacheName, err)
		}
		res = data
...
// return error / nil
func (rd *RedisClient) DeleteCache(ctx context.Context, conn *redis.Client, keys ...string) map[string]error {
